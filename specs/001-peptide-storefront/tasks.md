...
- [X] T001 Scaffold Next.js 16 App Router project with TypeScript and Tailwind CSS at repository root
...
- [X] T023 [P] [US2] Build `components/vial-selector.tsx`: vial choice control (radio/select, localized labels where needed) + quantity input constrained to `1 <= quantity <= 10` (data-model.md, per T045) — no price/subtotal calculation
...
- [X] T030 [US4] Extend `app/[locale]/quote/page.tsx` with the contact form (`customerName`, `customerEmail`, required verified `customerPhone`, full `shippingAddress`, optional `notes`; see T070) using React Hook Form + `@hookform/resolvers/zod` bound to `lib/quote-schema.ts`
...
**Purpose**: Bring the built code in line with the spec clarifications (spec.md "Clarifications" section, FR-004, FR-009, FR-011a, Edge Cases). Earlier tasks that mention a 1–20 quantity, optional phone, or country-only address (T014, T023, T030, T031) are superseded by these, by Phase 10 (T070) and by data-model.md.
...
- [X] T068 [P] [US4] Docs: update `README.md` and `CLAUDE.md` (local setup, new env vars, the dev OTP fallback and that it is disabled in production, the "no external service needed for local dev" statement stays true thanks to the fallback), and `specs/001-peptide-storefront/quickstart.md` only if the implementation differs from it
...
## Phase 10: Analysis Remediation (2026-09-20)
...
- **Analysis Remediation (Phase 10)**: Depends on Phases 1–9. T071–T072 in parallel; T073 needs a running app; T074 any time
...
```bash
Task: "Build components/vial-selector.tsx with quantity 1-10, no price/subtotal"
